...
fifa_ranking = pd.read_csv(os.path.join(DIR, "./ml_data/fifa_ranking.csv"))
# # # # # # # # # # # # #
...
def get_average_goals(home, away, year, ignore_sides=False):
    ...
    while i < len(matches) and matches["Year"][i] <= year:
        ...
        if ignore_sides and (
            home.lower() in matches["Away Team Name"][i].lower()
            and away.lower() in matches["Home Team Name"][i].lower()
        ):
    ...
    return [avg_home / n, avg_away / n] if n > 0 else False
